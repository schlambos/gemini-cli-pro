  ...
  describe('home directory check', () => {
    it('should return a warning when running in home directory', async () => {
      ...
      expect(warnings).toContainEqual(
        expect.stringContaining('Warning you are running Gemini CLI in your home directory')
      );
      expect(warnings).toContainEqual(expect.stringContaining('warning can be disabled in /settings'));
    });
    ...
    it('should not return a warning when showHomeDirectoryWarning is false', async () => {
      const warnings = await getUserStartupWarnings({ ui: { showHomeDirectoryWarning: false } }, homeDir);
      expect(warnings).not.toContainEqual(expect.stringContaining('home directory'));
    });
