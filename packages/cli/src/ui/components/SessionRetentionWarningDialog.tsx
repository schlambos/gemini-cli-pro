...
export const SessionRetentionWarningDialog = ({
  ...
  return (
    <Box borderStyle='round' borderColor={theme.border.default} flexDirection='column' width='100%' padding={1}>
      ...
      <Box marginTop={1}>
        <RadioButtonSelect items={options} onSelect={(action) => action()} initialIndex={1} />
      </Box>

      <Box marginTop={1}>
        <Text color={theme.text.secondary}>
          Set a custom limit <Text color={theme.text.primary}>/settings</Text> and change &quot;Keep chat history&quot;.
        </Text>
      </Box>
    </Box>
  );
