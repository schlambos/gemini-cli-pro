...
describe('<EnumSelector />', () => {
  it('renders with string options and matches snapshot', () => {
    const { lastFrame } = renderWithProviders(
      <EnumSelector options={LANGUAGE_OPTIONS} currentValue='en' isActive={true} onValueChange={() => {}} />
    );
  ...
  it('renders with numeric options and matches snapshot', () => {
    const { lastFrame } = renderWithProviders(
      <EnumSelector options={NUMERIC_OPTIONS} currentValue={2} isActive={true} onValueChange={() => {}} />
    );
    expect(lastFrame()).toMatchSnapshot();
  });
  ...
  it('renders inactive state and matches snapshot', () => {
    const { lastFrame } = renderWithProviders(
      <EnumSelector options={LANGUAGE_OPTIONS} currentValue='zh' isActive={false} onValueChange={() => {}} />
    );
    expect(lastFrame()).toMatchSnapshot();
  });

  it('renders with single option and matches snapshot', () => {
    const singleOption: readonly SettingEnumOption[] = [{ label: 'Only Option', value: 'only' }];
    const { lastFrame } = renderWithProviders(
      <EnumSelector options={singleOption} currentValue='only' isActive={true} onValueChange={() => {}} />
    );
    expect(lastFrame()).toMatchSnapshot();
  });
  ...
  it('renders nothing when no options are provided', () => {
    const { lastFrame } = renderWithProviders(
      <EnumSelector options={[]} currentValue='' isActive={true} onValueChange={() => {}} />
    );
  ...
  it('handles currentValue not found in options', () => {
    const { lastFrame } = renderWithProviders(
      <EnumSelector options={LANGUAGE_OPTIONS} currentValue='invalid' isActive={true} onValueChange={() => {}} />
    );
  ...
  it('updates when currentValue changes externally', () => {
    ...
    rerender(<EnumSelector options={LANGUAGE_OPTIONS} currentValue='zh' isActive={true} onValueChange={() => {}} />);
