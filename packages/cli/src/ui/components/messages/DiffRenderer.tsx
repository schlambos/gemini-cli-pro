...
export const DiffRenderer: React.FC<DiffRendererProps> = ({
  ...
  const renderedOutput = useMemo(() => {
    ...
    if (isNewFile) {
      ...
      return renderDiffContent(parsedLines, filename, tabWidth, availableTerminalHeight, terminalWidth);
    }
  }, [
    diffContent,
    parsedLines,
    ...
    acc.push(
      <Box key={lineKey} flexDirection='row'>
        ...
        {line.type === 'context' ? (
          ...
          <Text
            backgroundColor={
              line.type === 'add' ? semanticTheme.background.diff.added : semanticTheme.background.diff.removed
            }
            wrap='wrap'
          >
            <Text color={line.type === 'add' ? semanticTheme.status.success : semanticTheme.status.error}>
              {prefixSymbol}
            </Text>{' '}
