    ...
    return (
      <Box flexDirection='column'>
        ...
        <Text>
          ...
          <Text color={theme.text.link}>https://goo.gle/gemini-cli-docs-mcp</Text> or use the cli /docs command
        </Text>
      </Box>
    );
  }
  ...
  return (
    <Box flexDirection='column'>
      ...
      {serverNames.map((serverName) => {
        ...
        if (isDisabled) {
          statusIndicator = '⏸️';
          statusText = serverEnablement.isSessionDisabled ? 'Disabled (session)' : 'Disabled';
        ...
        if (serverAuthStatus === 'authenticated') {
          ...
          authStatusNode = <Text color={theme.status.warning}> (OAuth not authenticated)</Text>;
        }
        ...
        return (
          <Box key={serverName} flexDirection='column' marginBottom={1}>
            ...
            {status === MCPServerStatus.CONNECTING && <Text> (tools and prompts will appear when ready)</Text>}
            {status === MCPServerStatus.DISCONNECTED && toolCount > 0 && <Text> ({toolCount} tools cached)</Text>}
              ...
              <Box flexDirection='column' marginLeft={2}>
                  ...
                  return (
                    <Box key={`${resource.serverName}-resource-${index}`} flexDirection='column'>
                      ...
                      {showDescriptions && resource.description && (
                        <Box marginLeft={2}>
                          <Text color={theme.text.secondary}>{resource.description.trim()}</Text>
                        </Box>
                      )}
                    </Box>
                  );
                ...
                {serverResources.length > MAX_MCP_RESOURCES_TO_SHOW && (
                  <Text color={theme.text.secondary}>
                    {'  '}... {serverResources.length - MAX_MCP_RESOURCES_TO_SHOW}{' '}
                    {serverResources.length - MAX_MCP_RESOURCES_TO_SHOW === 1 ? 'resource' : 'resources'} hidden
                  </Text>
                )}
              </Box>
            )}
