    ...
    <Box flexDirection='column' marginBottom={1}>
      <Box flexDirection='column'>
        ...
        <Text color={theme.status.warning}>
          Hooks can execute arbitrary commands on your system. Only use hooks from sources you trust. Review hook
          scripts carefully.
        </Text>
      </Box>
      ...
      <Box marginTop={1}>
        <Text>
          Learn more: <Text color={theme.text.link}>https://geminicli.com/docs/hooks</Text>
        </Text>
      </Box>
          ...
          <Box key={eventName} flexDirection='column' marginBottom={1}>
            ...
            <Box flexDirection='column' paddingLeft={2}>
              {eventHooks.map((hook, index) => {
                ...
                const statusColor = hook.enabled ? theme.status.success : theme.text.secondary;
                ...
                return (
                  <Box key={`${eventName}-${index}`} flexDirection='column'>
                    ...
                    <Box paddingLeft={2} flexDirection='column'>
                      {hook.config.description && <Text italic>{hook.config.description}</Text>}
                      <Text dimColor>
                        Source: {hook.source}
      ...
      <Box marginTop={1}>
        <Text dimColor>
          Tip: Use <Text bold>/hooks enable {'<hook-name>'}</Text> or <Text bold>/hooks disable {'<hook-name>'}</Text>{' '}
          to toggle individual hooks. Use <Text bold>/hooks enable-all</Text> or <Text bold>/hooks disable-all</Text> to
          toggle all hooks at once.
        </Text>
      </Box>
    </Box>
  );
