 ...
 */
function calculateStreamingState(isResponding: boolean, toolCalls: TrackedToolCall[]): StreamingState {
  ...
  const isAnyToolActive = toolCalls.some((tc) => {
    ...
    if (
      ...
      return !(tc as TrackedCompletedToolCall | TrackedCancelledToolCall).responseSubmittedToGemini;
    }
...
export const useGeminiStream = (
  ...
  const [pushedToolCallIds, pushedToolCallIdsRef, setPushedToolCallIds] = useStateAndRef<Set<string>>(new Set());
  const [_isFirstToolInGroup, isFirstToolInGroupRef, setIsFirstToolInGroup] = useStateAndRef<boolean>(true);
  ...
  const [
    ...
    async (completedToolCallsFromScheduler) => {
      ...
      if (completedToolCallsFromScheduler.length > 0) {
        ...
        try {
          const currentModel = config.getGeminiClient().getCurrentSequenceModel() ?? config.getModel();
          config.getGeminiClient().getChat().recordCompletedToolCalls(currentModel, completedToolCallsFromScheduler);
  ...
  }, [toolCalls, pushedToolCallIdsRef, setPushedToolCallIds, setIsFirstToolInGroup, streamingState]);
  ...
  }, [toolCalls, pushedToolCallIdsRef, isFirstToolInGroupRef, setPushedToolCallIds, setIsFirstToolInGroup, addItem]);
  ...
  const pendingToolGroupItems = useMemo((): HistoryItemWithoutId[] => {
    ...
    const allTerminal =
      toolCalls.length > 0 &&
      toolCalls.every((tc) => tc.status === 'success' || tc.status === 'error' || tc.status === 'cancelled');
    ...
    const anyVisibleInPending = remainingTools.some((tc) => {
      ...
      return tc.status !== 'scheduled' && tc.status !== 'validating' && tc.status !== 'awaiting_approval';
    });

    if (toolCalls.length > 0 && !(allTerminal && allPushed) && (anyVisibleInHistory || anyVisibleInPending)) {
      items.push({
  ...
  const [loopDetectionConfirmationRequest, setLoopDetectionConfirmationRequest] = useState<{
  ...
  useEffect(() => {
    if (turnCancelledRef.current && prevActiveShellPtyIdRef.current !== null && activeShellPtyId === null) {
  ...
  useEffect(() => {
    if (config.getApprovalMode() === ApprovalMode.YOLO && streamingState === StreamingState.Idle) {
      const lastUserMessageIndex = history.findLastIndex((item: HistoryItem) => item.type === MessageType.USER);
  ...
  const cancelOngoingRequest = useCallback(() => {
    if (streamingState !== StreamingState.Responding && streamingState !== StreamingState.WaitingForConfirmation) {
      return;
    }
  ...
  const handleContentEvent = useCallback(
    (eventValue: ContentEvent['value'], currentGeminiMessageBuffer: string, userMessageTimestamp: number): string => {
      setRetryStatus(null);
      if (turnCancelledRef.current) {
  ...
  const handleChatCompressionEvent = useCallback(
    (eventValue: ServerGeminiChatCompressedEvent['value'], userMessageTimestamp: number) => {
      if (pendingHistoryItemRef.current) {
        addItem(pendingHistoryItemRef.current, userMessageTimestamp);
        setPendingHistoryItem(null);
      }
  ...
  const handleAgentExecutionStoppedEvent = useCallback(
    (reason: string, userMessageTimestamp: number, systemMessage?: string, contextCleared?: boolean) => {
      if (pendingHistoryItemRef.current) {
        addItem(pendingHistoryItemRef.current, userMessageTimestamp);
        setPendingHistoryItem(null);
      }
  ...
  const processGeminiStreamEvents = useCallback(
    async (
      ...
      for await (const event of stream) {
        if (event.type !== ServerGeminiEventType.Thought && thoughtRef.current !== null) {
          setThought(null);
        }
  ...
  const submitQuery = useCallback(
    async (query: PartListUnion, options?: { isContinuation: boolean }, prompt_id?: string) =>
      runInDevTraceSpan({ name: 'submitQuery' }, async ({ metadata: spanMetadata }) => {
    ...
    async (newApprovalMode: ApprovalMode) => {
      ...
      if (newApprovalMode === ApprovalMode.YOLO || newApprovalMode === ApprovalMode.AUTO_EDIT) {
        let awaitingApprovalCalls = toolCalls.filter(
          (call): call is TrackedWaitingToolCall => call.status === 'awaiting_approval'
        );
  ...
  const handleCompletedTools = useCallback(
    async (completedToolCallsFromScheduler: TrackedToolCall[]) => {
      ...
      const responsesToSend: Part[] = geminiTools.flatMap((toolCall) => toolCall.response.responseParts);
      const callIdsToMarkAsSubmitted = geminiTools.map((toolCall) => toolCall.request.callId);
  ...
  useEffect(() => {
    const saveRestorableToolCalls = async () => {
      ...
      const restorableToolCalls = toolCalls.filter(
        (toolCall) =>
          EDIT_TOOL_NAMES.has(toolCall.request.name) && toolCall.status === CoreToolCallStatus.AwaitingApproval
      );
  ...
  }, [toolCalls, config, onDebugMessage, gitService, history, geminiClient, storage]);
