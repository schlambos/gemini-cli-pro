...
describe('usePhraseCycler', () => {
  ...
  it('should show interactive shell waiting message immediately when isInteractiveShellWaiting is true', async () => {
    const { lastFrame, rerender } = render(<TestComponent isActive={true} isWaiting={false} />);
    rerender(<TestComponent isActive={true} isWaiting={false} isInteractiveShellWaiting={true} />);
    await act(async () => {
      await vi.advanceTimersByTimeAsync(0);
    });
    expect(lastFrame()).toMatchSnapshot();
  ...
  it('should prioritize interactive shell waiting over normal waiting immediately', async () => {
    ...
    rerender(<TestComponent isActive={true} isWaiting={true} isInteractiveShellWaiting={true} />);
    await act(async () => {
      await vi.advanceTimersByTimeAsync(0);
    });
    expect(lastFrame()).toMatchSnapshot();
  ...
  it('should fall back to witty phrases if custom phrases are an empty array', async () => {
    vi.spyOn(Math, 'random').mockImplementation(() => 0.5); // Always witty for subsequent phrases
    const { lastFrame } = render(<TestComponent isActive={true} isWaiting={false} customPhrases={[]} />);
    await act(async () => {
