...
function checkRelease() {
  try {
    ...
    const gcloudOutput = execSync(`gcloud storage ls --recursive ${bucketUri}`, { encoding: 'utf-8' });
    const files = gcloudOutput.trim().split('\n');
    const vsixFiles = files.filter((file) =>
      /signed-gemini-cli-vscode-ide-companion-\d+\.\d+\.\d+-[a-f0-9]{7}\.vsix$/.test(file)
    );
    ...
    const match = /signed-gemini-cli-vscode-ide-companion-(\d+\.\d+\.\d+)-([a-f0-9]{7})\.vsix$/.exec(fileName);

    if (!match || !match[1] || !match[2]) {
      console.error(`Could not extract version and commit hash from filename: ${fileName}`);
      process.exit(1);
    }
    ...
    console.log('\n--- Summary ---');
    if (gitLog) {
      console.log('New commits found for `packages/vscode-ide-companion` since last release. A new release is needed.');
    } else {
      console.log('No new commits found since last release. No release is necessary.');
    }

    if (noticesDiff) {
      console.log('Dependencies have changed. The license review form will require extra details.');
    } else {
