...
async function getDependencyLicense(depName, depVersion) {
  ...
  try {
    const localDepPath = path.join(packagePath, 'node_modules', depName, 'package.json');
...
function collectDependencies(packageName, packageLock, dependenciesMap, workspaceRelativePath) {
  if (dependenciesMap.has(packageName)) {
    return;
  }

  let packageInfo = packageLock.packages[`${workspaceRelativePath}/node_modules/${packageName}`];
  if (!packageInfo) {
...
async function main() {
  try {
    ...
    for (const depName of directDependencies) {
      collectDependencies(depName, packageLockJson, allDependencies, workspaceRelativePath);
    }

    const dependencyEntries = Array.from(allDependencies.entries());

    const licensePromises = dependencyEntries.map(([depName, depVersion]) => getDependencyLicense(depName, depVersion));

    const dependencyLicenses = await Promise.all(licensePromises);

    let noticeText = 'This file contains third-party software notices and license terms.\n\n';
