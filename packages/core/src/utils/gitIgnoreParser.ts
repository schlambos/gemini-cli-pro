...
export class GitIgnoreParser implements GitIgnoreFilter {
  ...
  private loadPatternsForFile(patternsFilePath: string): Ignore {
    ...
    const relativeBaseDir = isExcludeFile
      ? '.'
      : path.dirname(path.relative(this.projectRoot, patternsFilePath)).split(path.sep).join(path.posix.sep);
  ...
  isIgnored(filePath: string): boolean {
    ...
    try {
      ...
      if (this.globalPatterns === undefined) {
        ...
        this.globalPatterns = fs.existsSync(excludeFile) ? this.loadPatternsForFile(excludeFile) : ignore();
      }
      ...
      for (const dir of dirsToVisit) {
        ...
        if (relativeDir) {
          ...
          const igPlusExtras = ignore().add(ig).add(this.processedExtraPatterns); // takes priority over ig patterns
