...
describe('FileSearch', () => {
  ...
  it('should use .geminiignore rules', async () => {
    ...
    expect(results).toEqual(['src/', GEMINI_IGNORE_FILE_NAME, 'src/not-ignored.js']);
  });
  ...
  it('should combine .gitignore and .geminiignore rules', async () => {
    ...
    expect(results).toEqual(['src/', GEMINI_IGNORE_FILE_NAME, '.gitignore', 'src/not-ignored.js']);
  });
  ...
  it('should leverage ResultCache for bestBaseQuery optimization', async () => {
    ...
    expect(broadResults).toEqual(['src/', 'src/nested/', 'src/bar.ts', 'src/foo.js', 'src/nested/baz.js']);
