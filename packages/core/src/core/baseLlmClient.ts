...
export class BaseLlmClient {
   ...
   */
  private async generateJsonForOpenAI(options: GenerateJsonOptions): Promise<Record<string, unknown>> {
    const { modelConfigKey, contents, schema, abortSignal, systemInstruction, promptId, role, maxAttempts } = options;
    ...
    try {
      ...
      if (!text) {
        const error = new Error('API returned an empty response for generateJson (OpenAI).');
        await reportError(
          error,
      ...
      await reportError(error, 'Error generating JSON content via OpenAI API.', contents, 'generateJson-openai-api');
  ...
  private async _generateWithRetry(
    ...
    const { modelConfigKey, contents, systemInstruction, abortSignal, promptId, maxAttempts, additionalProperties } =
      options;
