...
describe('PolicyEngine', () => {
  ...
  describe('check', () => {
    ...
    it('should match current tool call against legacy tool name rules', async () => {
      ...
      const rules: PolicyRule[] = [{ toolName: legacyName, decision: PolicyDecision.DENY }];
    ...
    it('should match legacy tool call against current tool name rules (for skills support)', async () => {
      ...
      const rules: PolicyRule[] = [{ toolName: currentName, decision: PolicyDecision.ALLOW }];
    ...
    it('should match tool call using one legacy name against policy for another legacy name (same canonical tool)', async () => {
      ...
      const rules: PolicyRule[] = [{ toolName: legacyName2, decision: PolicyDecision.DENY }];
  ...
  describe('MCP server wildcard patterns', () => {
    it('should match MCP server wildcard patterns', async () => {
      ...
      expect((await engine.check({ name: 'my-server__another_tool' }, undefined)).decision).toBe(PolicyDecision.ALLOW);

      // Should match blocked-server tools
      expect((await engine.check({ name: 'blocked-server__tool1' }, undefined)).decision).toBe(PolicyDecision.DENY);
      expect((await engine.check({ name: 'blocked-server__dangerous' }, undefined)).decision).toBe(PolicyDecision.DENY);

      // Should not match other patterns
      expect((await engine.check({ name: 'other-server__tool' }, undefined)).decision).toBe(PolicyDecision.ASK_USER);
    ...
    it('should prioritize specific tool rules over server wildcards', async () => {
      ...
      expect((await engine.check({ name: 'my-server__dangerous-tool' }, undefined)).decision).toBe(PolicyDecision.DENY);
      expect((await engine.check({ name: 'my-server__safe-tool' }, undefined)).decision).toBe(PolicyDecision.ALLOW);
    });
    ...
    it('should NOT match spoofed server names when using wildcards', async () => {
      ...
      expect((await engine.check(spoofedToolCall, 'safe_server__malicious')).decision).toBe(PolicyDecision.ASK_USER);
    });
  ...
  describe('complex scenarios', () => {
    it('should handle multiple matching rules with different priorities', async () => {
      ...
      expect((await engine.check({ name: 'shell', args: { command: 'ls -la' } }, undefined)).decision).toBe(
        PolicyDecision.ALLOW
      );

      // Matches middle priority rule (shell without ls)
      expect((await engine.check({ name: 'shell', args: { command: 'pwd' } }, undefined)).decision).toBe(
    ...
    it('should handle tools with no args', async () => {
      ...
      expect((await engine.check({ name: 'read', args: { file: 'public.txt' } }, undefined)).decision).toBe(
        PolicyDecision.ASK_USER
      );

      // Tool call with args matching pattern
      expect((await engine.check({ name: 'read', args: { file: 'secret.txt' } }, undefined)).decision).toBe(
        PolicyDecision.DENY
      );
    });
    ...
    it('should match args pattern regardless of property order', async () => {
      ...
      expect((await engine.check({ name: 'shell', args: args1 }, undefined)).decision).toBe(PolicyDecision.DENY);
      expect((await engine.check({ name: 'shell', args: args2 }, undefined)).decision).toBe(PolicyDecision.DENY);

      // Verify safe command doesn't match
      const safeArgs = { command: 'ls -la', path: '/home' };
      expect((await engine.check({ name: 'shell', args: safeArgs }, undefined)).decision).toBe(PolicyDecision.ASK_USER);
    });
    ...
    it('should handle circular references without stack overflow', async () => {
      ...
      expect((await engine.check({ name: 'test', args: circularArgs }, undefined)).decision).toBe(PolicyDecision.DENY);
    ...
    it('should handle deep circular references', async () => {
      ...
      expect((await engine.check({ name: 'deep', args: deepCircular }, undefined)).decision).toBe(PolicyDecision.DENY);
    });
    ...
    it('should produce valid JSON for all inputs', async () => {
      ...
      for (const { input } of testCases) {
        ...
        expect((await engine.check({ name: 'test', args: input }, undefined)).decision).toBe(PolicyDecision.ALLOW);
      }
    });
    ...
    it('should downgrade ALLOW to ASK_USER for redirected shell commands', async () => {
      ...
      expect(
        (await engine.check({ name: 'run_shell_command', args: { command: 'echo "hello"' } }, undefined)).decision
    ...
    it('should report redirection when a sub-command specifically has redirection', async () => {
      ...
      vi.mocked(splitCommands).mockReturnValueOnce(['mkdir bar', 'echo hello > bar/test.md']);
