 ...
 */
export function getPolicyDirectories(defaultPoliciesDir?: string, policyPaths?: string[]): string[] {
  const dirs: string[] = [];
 ...
 */
async function filterSecurePolicyDirectories(dirs: string[]): Promise<string[]> {
...
export function createPolicyUpdater(policyEngine: PolicyEngine, messageBus: MessageBus) {
  ...
  messageBus.subscribe(MessageBusType.UPDATE_POLICY, async (message: UpdatePolicy) => {
    ...
    if (message.commandPrefix) {
      ...
      const argsPattern = message.argsPattern ? new RegExp(message.argsPattern) : undefined;
