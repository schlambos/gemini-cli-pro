...
describe('patch-create-comment', () => {
  ...
  describe('Environment flag', () => {
    ...
    it('fails if the ENVIRONMENT is bogus', () => {
      ...
      expect(result.stderr).toContain('Argument: environment, Given: "totally-bogus", Choices: "prod", "dev"');
    });
...
\`\`\``,
      ...
      expect(result.stdout).toContain('Please run these commands manually to create the release branch:');
      expect(result.stdout).toContain('git checkout -b hotfix/v0.4.1/stable/cherry-pick-abc1234 v0.4.1');
      expect(result.stdout).toContain('git push origin hotfix/v0.4.1/stable/cherry-pick-abc1234');
    });
  });
